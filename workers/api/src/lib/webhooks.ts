...
export async function fireWebhooks(
  ...
  const rows = await env.iai_flow_db
    .prepare(
      `SELECT w.id, w.webhook_url, c.webhook_key
       FROM n8n_webhooks w
       LEFT JOIN n8n_connections c ON c.id = w.connection_id
       WHERE w.trigger_event = ?1 AND w.is_active = 1`
    )
    .bind(event)
    .all<{ id: number; webhook_url: string; webhook_key: string | null }>()

  for (const row of rows.results ?? []) {
    ctx.waitUntil(sendWebhook(env, row.id, row.webhook_url, { event, source: "nhachung.org", ...payload }, row.webhook_key))
  }
}
