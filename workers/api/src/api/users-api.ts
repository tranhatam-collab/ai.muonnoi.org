...
export async function handleUserProfile(
  ...
  if (!user) return json({ ok: false, error: "Không tìm thấy người dùng" }, 404, origin, env)
...
export async function handleFollow(
  ...
  if (user.id === Number(targetUserId)) {
    return json({ ok: false, error: "Không thể tự follow" }, 400, origin, env)
  }

  const targetUser = await findUserById(env, Number(targetUserId))
  if (!targetUser) return json({ ok: false, error: "Không tìm thấy người dùng" }, 404, origin, env)
  ...
  if (existing) {
    ...
    return json({ ok: true, data: { following: false } }, 200, origin, env)
  }
  ...
  return json({ ok: true, data: { following: true } }, 200, origin, env)
}
