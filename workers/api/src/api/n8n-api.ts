...
export async function handleN8nConnections(request: Request, env: Env): Promise<Response> {
  ...
  if (method === "POST") {
    ...
    if (!baseUrl) return json({ ok: false, error: "n8n_base_url là bắt buộc" }, 400, origin, env)
    if (!isValidHttpUrl(baseUrl)) return json({ ok: false, error: "n8n_base_url phải là http hoặc https" }, 400, origin, env)
    ...
    return json({ ok: true, data: { id: result.meta?.last_row_id, name, n8n_base_url: baseUrl, webhook_key: key } }, 201, origin, env)
  }
...
export async function handleN8nWebhooks(request: Request, env: Env): Promise<Response> {
  ...
  if (method === "POST") {
    ...
    if (!webhookUrl || !triggerEvent) return json({ ok: false, error: "webhook_url và trigger_event là bắt buộc" }, 400, origin, env)
    if (!isValidHttpUrl(webhookUrl)) return json({ ok: false, error: "webhook_url phải là http hoặc https" }, 400, origin, env)
    ...
    if (!validEvents.includes(triggerEvent)) {
      return json({ ok: false, error: `trigger_event phải là một trong: ${validEvents.join(", ")}` }, 400, origin, env)
    }
    ...
    return json({ ok: true, data: { id: result.meta?.last_row_id, name, webhook_url: webhookUrl, trigger_event: triggerEvent } }, 201, origin, env)
  }
...
export async function handleN8nWebhookById(
  ...
  if (!hook || hook.user_id !== user.id) return json({ ok: false, error: "Không tìm thấy" }, 404, origin, env)
...
export async function handleN8nWebhookTest(
  ...
  const hook = await env.iai_flow_db
    .prepare(
      `SELECT w.user_id, w.webhook_url, w.trigger_event, c.webhook_key
       FROM n8n_webhooks w
       LEFT JOIN n8n_connections c ON c.id = w.connection_id
       WHERE w.id = ?1
       LIMIT 1`
    ...
    .bind(webhookId)
    .first<{ user_id: number; webhook_url: string; trigger_event: string; webhook_key: string | null }>()
  ...
  try {
    const res = await fetch(hook.webhook_url, {
      ...
      headers: {
        ...
        ...(hook.webhook_key ? { "X-Webhook-Key": hook.webhook_key } : {})
      },
